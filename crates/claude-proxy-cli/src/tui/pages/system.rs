...
pub fn render_system_page(f: &mut Frame, app: &App, area: Rect) {
    ...
    let log_path = app
        ...
        .unwrap_or_else(|| {
            claude_proxy_config::Settings::config_dir()
                .map(|p| p.join("logs").join("claude-proxy.log").display().to_string())
    ...
    render_info_rows(f, runtime_area, &[
        ("Version", version),
        ("PID", &pid),
        ("Server", &server_addr),
        ("Status", status),
    ...
    if rows[2].height >= 4 {
        ...
        render_info_rows(f, env_area, &[
            ("OS", &format!("{os}/{arch}")),
            ("RUST_LOG", &rust_log),
            ("Log Level", &app.settings.log.level),
            ("Log Stdout", if app.settings.log.with_stdout { "on" } else { "off" }),
        ]);
    }
}
