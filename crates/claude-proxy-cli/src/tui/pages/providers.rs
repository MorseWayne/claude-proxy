...
fn render_empty_state(f: &mut Frame, app: &App, area: Rect) {
    ...
    let text = vec![
        ...
        Line::from(""),
        Line::from(Span::styled("  Add a provider to get started.", subtitle_style)),
        Line::from(""),
        Line::from(vec![
            Span::raw("  "),
...
fn render_provider_list(f: &mut Frame, app: &App, area: Rect) {
    let is_focused = matches!(app.focus, Focus::Content)
        && matches!(app.provider_focus, ProviderFocus::List);
...
fn render_provider_detail(f: &mut Frame, app: &App, area: Rect) {
    let detail_focused = matches!(app.focus, Focus::Content)
        && matches!(app.provider_focus, ProviderFocus::Detail);
    ...
    let idx = app.content_idx.min(app.settings.providers.len().saturating_sub(1));
    ...
    let key_display = if is_copilot { "OAuth (auto)" } else { &cfg.api_key };
    ...
    // Proxy (detail_idx == 2)
    let proxy_display = if cfg.proxy.is_empty() { "(none)" } else { &cfg.proxy };
    widgets::render_field(
        f, rows[2], "Proxy",
        proxy_display,
    ...
    // Status (read-only)
    let is_default = app.settings.model.default.starts_with(&format!("{id}/"));
    let status = if is_default { format!("Default ({})", app.settings.model.default) } else { "Not default".into() };
    ...
    {
        let info = format!("oauth={} small={} warmup={}", cc.oauth_app, cc.small_model, cc.enable_warmup);
    ...
    let hints = if detail_focused {
        vec![
            Span::styled(" e ", Style::default().fg(theme::BG_DARK).bg(theme::ACCENT2).add_modifier(Modifier::BOLD)),
            Span::styled(" Edit  ", widgets::dim_style()),
            Span::styled(" ← ", Style::default().fg(theme::BG_DARK).bg(theme::ACCENT2).add_modifier(Modifier::BOLD)),
            Span::styled(" Back to list", widgets::dim_style()),
        ]
    } else {
        vec![
            Span::styled("  Press → or Enter to edit fields", widgets::dim_style()),
        ]
    };
    f.render_widget(
